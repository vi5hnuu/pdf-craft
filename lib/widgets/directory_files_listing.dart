...
class _DirectoryFilesListingState extends State<DirectoryFilesListing> {
    ...
    return PopScope(
      ...
      child: BlocConsumer<FilesBloc, FilesState>(
        listener: (context, state) {
          final error = state.getError(forr: HttpStates.loadDirectoryFiles);
          if (error != null) {
        ...
        builder: (context, state) {
          ...
          return Stack(children: [
            if (!state.isLoading(forr: HttpStates.loadDirectoryFiles))
              (state.files.isEmpty
                          ...
                          child: sortedFiles.isEmpty
                            ...
                            itemBuilder: (context, index) {
                              ...
                              if (deletedFiles.contains(file)) {
                                ...
                                if (state.isLoading(forr: HttpStates.deleteFile) ||
                                    state.isLoading(forr: HttpStates.moveFileTo)) {
                                  return const SizedBox.shrink();
                                }
  ...
  _loadDirectoryFiles(String path) {
    if (bloc.state.isLoading(forr: HttpStates.loadDirectoryFiles)) return;
  ...
  Future<void> _copyOrMove(File file, {required bool move}) async {
    final destDir = await showDialog<String>(
      context: context,
      builder: (_) => const _FolderPickerDialog(startPath: Constants.rootStoragePath),
    );
