...
class _ExtractEmbeddedFilesViewState extends State<ExtractEmbeddedFilesView>
  ...
  void initState() {
    ...
    resetToolState([HttpStates.extractEmbedded]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocConsumer<PdfBloc, PdfState>(
        buildWhen: (p, c) => p.httpStates[HttpStates.extractEmbedded] != c.httpStates[HttpStates.extractEmbedded],
        listenWhen: (p, c) => p.httpStates[HttpStates.extractEmbedded] != c.httpStates[HttpStates.extractEmbedded],
        listener: (context, state) => handleToolState(
          state.httpStates[HttpStates.extractEmbedded],
          successMessage: L10n.of(context).embeddedFilesExtracted,
          onDone: (f) => OpenFile.open(f.path),
        ),
        builder: (context, state) {
          final loading = state.httpStates[HttpStates.extractEmbedded]?.loading == true;
            ...
            Column(children: [
              Expanded(
                child: Padding(
                  ...
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    Text(L10n.of(context).extractAttachments,
              ...
              Container(
                ...
                child: FilledButton.icon(
                  ...
                  label: Text(L10n.of(context).extractFilesZip),
                ),
              ),
            ]),
            processingOverlay(state.httpStates[HttpStates.extractEmbedded], label: L10n.of(context).procWorking),
          ]);
        },
      ),
    );
